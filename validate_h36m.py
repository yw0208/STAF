...
from lib.dataset import ThreeDPW, Human36M
...
if __name__ == '__main__':
    ...
    print('init weight...')
    checkpoint = torch.load('experiments/27-11-2022_13-50-34_MPSnet/model_best.pth.tar')
    generator.load_state_dict(checkpoint['gen_state_dict'], strict=False)

    valid_db = Human36M(load_opt='repr_table6_h36m_model', set='test', seqlen=9, overlap=8.0 / 9.0, debug=False)
    ...
    joblib.dump(pred_j3ds, 'pred_j3dno.pt')
    joblib.dump(target_j3ds, 'target_j3dno.pt')
    print('saved successfully')

    pred_j3ds = joblib.load('pred_j3dno.pt')
    target_j3ds = joblib.load('target_j3dno.pt')
