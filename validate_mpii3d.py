...
from lib.dataset import ThreeDPW, Human36M, MPII3D
...
if __name__ == '__main__':
    ...
    print('init weight...')
    checkpoint = torch.load('experiments/28-11-2022_15-05-48_MPSnet/model_best.pth.tar')
    generator.load_state_dict(checkpoint['gen_state_dict'], strict=False)

    valid_db = MPII3D(load_opt='repr_table4_h36m_mpii3d_model', set='val', seqlen=9, overlap=8.0/9.0, debug=False)
    ...
    for i, target in enumerate(valid_loader):
        ...
        with torch.no_grad():
            ...
            preds,_ = generator(inp, J_regressor=None)  #

            # convert to 14 keypoint format for evaluation
            n_kp = preds[-1]['kp_3d'].shape[-2]
            pred_j3d = preds[-1]['kp_3d'].view(-1, n_kp, 3).cpu().numpy()
            target_j3d = target['kp_3d'].view(-1, 17, 3).cpu().numpy()
