...
from lib.models import SAFM, MotionDiscriminator
...
def main(cfg):
    if cfg.SEED_VALUE >= 0:
        ...
        random.seed(cfg.SEED_VALUE)  # 会让随机生成数根据输入而不是时间，来生成固定的随机数列
        torch.manual_seed(cfg.SEED_VALUE)  # 同上
    ...
    print('init weight...')
    checkpoint = torch.load('data/base_data/base_model.pt')
