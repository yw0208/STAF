...
class Dataset3D(Dataset):
    ...
    def load_db(self):
        ...
        db_file='/home/yaowei/project/PyMAF-moca-mps/data/preporcessd_data/3dpw_basketball.pt'
        if self.set == 'train':
            if self.load_opt == 'repr_table4_3dpw_model':
                ...
                if self.dataset_name == '3dpw':
                    ...
                    db_file = osp.join(TCMR_DB_DIR, f'{self.dataset_name}_{self.set}_25fps_full_nosmpl_db.pt')
        ...
        elif self.set == 'test' and self.dataset_name == 'h36m':
            if self.load_opt == 'repr_table4_h36m_mpii3d_model':
                db_file = osp.join('/home/yaowei/project/PyMAF-moca-mps/data/preporcessd_data/3dpw_line_chart.pt')
            ...
            db_file = osp.join(TCMR_DB_DIR, f'{self.dataset_name}_val_front_25fps_tight_nosmpl_db.pt')
    ...
    def get_single_item(self, index):
        ...
        # )
        names = self.get_sequence(start_index, end_index, self.db['img_name'])
        vid_names = self.get_sequence(start_index, end_index, self.db['vid_name'])
        path_list = self.get_path_list(vid_names, names)
    ...
    def get_path_list(self, vid_names, img_names):
        img_path_list = []
        for vid_name, img_name in zip(vid_names, img_names):
            if 'h36m' in img_name:
                path = '/opt/data/private/datasets/h36m_img_feature'
                video = img_name.split('/')[-2]
                image = img_name.split('/')[-1].split('.')[0]

            elif '3dpw' in img_name:
                path = '/home/yaowei/3dpw_crop_img_feature/imageFiles'
                video = vid_name
                image = img_name.split('/')[-1].split('.')[0]

            else:
                vid_name_list = vid_name.split('_')
                if len(vid_name_list) == 4:
                    path = '/opt/data/private/datasets/mpii_3d_img_feature'
                else:
                    path = '/opt/data/private/datasets/mpii_3d_val_img_feature'
                image = img_name.split('/')[-1].split('.')[0]
                video = '/'.join(vid_name_list)

            img_path_list.append(osp.join(path, video, image) + '.pt')
