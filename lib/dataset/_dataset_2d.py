...
class Dataset2D(Dataset):
    ...
    def get_path_list(self, vid_names, frame_ids):
        ...
        for vid_name, frame_id in zip(vid_names, frame_ids):
            path = '/opt/data/private/datasets/posetrack_img_feature'
            video = vid_name.split('/')[-1]
            img = frame_id.split('/')[-1].split('.')[0] + '.pt'
            feature_path = os.path.join(path, video, img)
            img_path_list.append(feature_path)
