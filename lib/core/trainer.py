    ...
    def train(self):
        ...
        for i in range(self.num_iters_per_epoch):
            ...
            if target_2d and target_3d:
                inp = torch.cat((target_2d['imgs'], target_3d['imgs']), dim=0).cuda()
            elif target_3d:
                inp = target_3d['imgs'].cuda()
            else:
                inp = target_2d['imgs'].cuda()

            timer['data'] = time.time() - start
            start = time.time()

            # preds, scores, preds_Dm = self.generator(inp, mid_inp, is_train=True)
